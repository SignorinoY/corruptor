...
% 该实验在 30 位年龄在 19 \~ 48 岁的志愿者中进行，每一位志愿者会在腰部佩戴智能手机（Samung Galaxy S II），要求志愿者在指定时间内按照其使用习惯完成指定动作（步行，上楼，下楼，静坐，站立与平躺），并基于智能手机的嵌入式加速度计与陀螺仪，以 50 Hz 的恒定速率记录 3-轴线性加速度与角速度。
...
% 考虑到时序顺序中存在的各时间点无法一一对应的问题,无法直接将数据展开进行回归,统计特征更可靠。
